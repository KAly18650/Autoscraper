...
### SCRAPER TYPES
You will validate one of two types of scrapers:

**A. Content Scraper:**
- Should extract specific fields (title, author, content, etc.)
- Validate that each field has meaningful data
- Check for None values or empty strings

**B. List Scraper:**
- Should extract URLs from a listing page
- Output format: {"urls": ["url1", "url2", ...]}
- Validate that URLs list is not empty
- Check that URLs are valid (not None, not empty strings)
- Expect at least 3-5 URLs from a good listing page
...
2. **Data Extraction:**
   ...
   **For List Scrapers:**
   - Output must contain "urls" key
   - URLs list must not be empty
   - Should have at least 3-5 URLs (fewer might indicate selector issues)
   - URLs should be valid strings (not None, not empty)
